  ...
  let getNumber = function () {
    ...
    if (userNumber === null) {
      ...
      if (userNumber < numberForty) {
        claim = confirm('Неизвестное число больше, осталось попыток ' + counterTry(setValue));

        incorrectAnswer(claim);

      } else if (userNumber > numberForty) {
        claim = confirm('Неизвестное число меньше, осталось попыток ' + counterTry(setValue));
        incorrectAnswer(claim);
      }
